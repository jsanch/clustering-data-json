...
def k_means_cluster(tfidf_matrix,dataLists,vocab_frame, terms,dist, num_clusters=10, num_fits=20):
    ...
    ## PRINTING TO COUT
    print('"Cluster Number", "keywords", "Dataset Title"')
    for i in range(num_clusters):
        ...
        for ind in order_centroids[i, :6]: #replace 6 with n words per cluster
            ...
            keywords = keywords + vocab_frame.ix[terms[ind].split(' ')].values.tolist()[0][0].encode('utf-8', 'ignore') + ', '
        # print(keywords) #add whitespace
        # print() #add whitespace
        for title in frame.ix[i]['title'].values.tolist():
            # print(' %s,' % title, end='\n')
            print('"' + str(i).encode('utf-8', 'ignore') + '", ' + '"' + keywords.rstrip(', ').encode('utf-8', 'ignore') + '"' + ", " + '"' + title.encode('utf-8', 'ignore')+  '"')
